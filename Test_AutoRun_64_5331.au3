...
Global $TBInstallPath	=	"C:\Users\Administrator\Documents\tbv5331_x64_portable"
...
Func TradeStar()
	  ...
	  ;WinWaitActive("交易开拓者平台(旗舰版) 64位 -","")
        ...
        If ControlListView ( "交易开拓者平台(旗舰版)", "", "SysListView321", "GetItemCount") >= 1	Then
                ...
                Filewriteline($LogName, "3:" & @TAB & $LogTime & "启动自动交易")
			 EndIf
			 Sleep(3000)
EndFunc
;动作编号4：启动监控
Func MonitorStar()
        ...
        If ControlListView ( "交易开拓者平台(旗舰版)" ,"", "SysListView321", "GetItemCount") >= 1	Then
				...
				ControlClick("自动交易头寸监控器","",2316)
			    Sleep(500)
                Filewriteline($LogName, "4:" & @TAB & $LogTime & "启动监控器")
			 EndIf
...
Func MonitorClose()
        if WinExists("自动交易头寸监控器", "") Then
                ...
                If WinExists("确认") Then
                        ...
                        Filewriteline($LogName, "6:" & @TAB & $LogTime & "关闭监控器")
                        Sleep(3000)
                EndIf
        EndIf
EndFunc
...
Func TradeStop()
        if WinExists("交易开拓者平台(旗舰版) 64位 -") Then
                ...
                Filewriteline($LogName, "7:" & @TAB & $LogTime & "停止自动交易")
                Sleep(3000)
        EndIf
EndFunc
;动作编号8：关闭TB
...
Func AccountLogin()
        if ControlListView ( "交易开拓者平台(旗舰版)", "", "SysListView321", "GetItemCount") < 1 Then
                ...
                Filewriteline($LogName, "10:" & @TAB & $LogTime & "账户登录（手动登）")
        EndIf
EndFunc
;动作编号11：登录失败
Func LoginFail()
        If ControlListView ( "交易开拓者平台(旗舰版)", "", "SysListView321", "GetItemCount") < 1 Then
                Filewriteline($LogName, "11:" & @TAB & $LogTime & "登录失败（节假日）")
                TBClose()
        EndIf
EndFunc
;动作编号20：重启系统
Func ShutdownR()
        Filewriteline($LogName, "20:" & @TAB & $LogTime & "周日重启系统")
        Run("shutdown -r")
EndFunc
;动作编号30：发邮件
Func SendEmail()
   Local $FromAddress="Gentle_TB@163.com"
   Local $ToAddress="sqltxt@qq.com"
   Local $SmtpServer="smtp.163.com"
   Local $UserName="Gentle_TB"
   Local $Password="nhh109620"
   Local $Subject="test邮件标题"
   Local $filename=""
   Local $Body="邮件内容"

        ;$sCommand = $TBInstallPath&"\SendEmail.exe -f "&$FromAddress&" -t "&$ToAddress&" -s "&$SmtpServer&" -xu "&$UserName&" -xp "&$Password&" -u "&$Subject&" -a "&$filename&" -o message-file="&$Body
	  _RunDos($TBInstallPath&"\SendEmail.exe -f "&$FromAddress&" -t "&$ToAddress&" -s "&$SmtpServer&" -xu "&$UserName&" -xp "&$Password&" -u "&$Subject&" -a "&$filename&" -o message-file="&$Body)
EndFunc
